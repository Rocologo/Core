...
import one.lindegaard.BagOfGoldCore.Messages;
...
public class ReloadCommand implements ICommand {
	...
	public ReloadCommand(BagOfGoldCore plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, String label, String[] args) {
		...
		if (plugin.getConfigManager().loadConfig()) {
			...
			if (n > 0) {
				plugin.getMessages().debug("Reloading %s PlayerSettings & PlayerBalances from the database", n);
