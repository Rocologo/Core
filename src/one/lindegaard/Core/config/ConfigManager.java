...
public class ConfigManager extends AutoConfig {
	...
	public ConfigManager(BagOfGoldCore plugin, File file) {
		...
		setCategoryComment("dropmoneyonground",
				"########################################################################"
						+ "\nDropMoneyOnGround Settings / BagofGoldCore Item Settings"
						+ "\n########################################################################");
	...
	public void backupConfig(File mFile) {
		...
		while (backupFile.exists() && count++ < 1000) {
			backupFile = new File("plugins/Core/backup/" + mFile.getName() + ".bak" + count);
		}
		if (mFile.exists())
			try {
				...
				Bukkit.getConsoleSender().sendMessage(ChatColor.GOLD + "[BagOfGold]" + ChatColor.RED
						+ "[ERROR] - Could not backup config.yml file to plugins/Core/config.yml. Delete some old backups");
				e1.printStackTrace();
			}
	}
